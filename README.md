...
## Conclusions
During the first few iterations of the model, the primary difficulties encountered were 1) the oscillating cross-validation loss/accuracy between epochs and 2) the relatively low cross-validation accuracy of the model. Regarding the first problem, it was believed that the oscillation originated from the extreme underrepresentation of the ModerateDemented class. This may have resulted in a fluctuating frequency of ModerateDemented cases selected for each batch, and combined with the large class weight assigned to ModerateDemented, this class may have exerted a large influence on parameter updates. To alleviate this issue, the learning rate was reduced ten times (from 0.001 to 0.0001), gradient clipping and a validation learning rate scheduler were implemented, and a stratified val/training split was used, amongst other changes. Additionally, to reduce the probability of overfitting, AdaptiveAvgPool2D was incorporated to reduce each feature map from the last block to a 1x1 spatial representation, reducing the size of the vector input into the linear layers and thus reducing the overall complexity of the model. Although these changes resulted in a slight increase in validation and training accuracy (from about ~50% to ~60%), it was discovered that by increasing the number of epochs from 20 to 50, the oscillation diminished in magnitude while both the validation and training accuracy dramatically increased, suggesting that the early instability and low accuracy observed could potentially be due to the model not sufficiently converging, rather than the architectural changes made. 

Subsequently, the primary goal of the project for the later versions of the model was to distinguish the VeryMildDemented from the NonDemented class. Multiple different modifications were evaluated using a trial-and-error strategy, including label smoothing, jitter augmentation removal, focal loss, an additional 256-filter block, and fractional pooling (detailed reasons explaining why these changes were considered can be found in the logbook). The results of the experiments guided the selection of the additional block, focal loss, and fractional pooling into the final version of the model. Additionally, for a more accurate validation F1 score, k-fold validation was implemented. Finally, because the original split of the training/test set may have contained specific distributional biases, this version of the model was subjected to five other different random train/test set splits to evaluate model performance. The final results demonstrated that although the macro validation F1 scores stayed relatively consistent between 0.95 and 0.80, the macro test F1 scores had a substantially high variability (0.6170 ± 0.2246). Specifically, the model confused a substantial number of VeryMildDemented cases for either NonDemented cases (seed 789) or ModerateDemented images (seed 456). This could be due to the addition of the weighted sampler, which, due to the extreme rarity of the ModerateDemented class compared to the VeryMildDemented class, could have disproportionally increased the frequency of ModerateDemented samples presented to the model. Consequently, focal loss may have emphasized the difficult ModerateDemented samples rather than emphasizing the VeryMildDemented class as intended. The addition of the extra block and fractional pooling could have exacerbated this issue by allowing the model to retain and utilize a greater amount of feature information, most of which was influenced by the aggressive updates. Thus, the model's dependency on the training/test split could be potentially due to the fluctuating distribution of the ModerateDemented class between random seeds, which, depending on the frequency of the ModerateDemented images that appeared in the training and test set, could have increased or decreased the severity of this issue.
