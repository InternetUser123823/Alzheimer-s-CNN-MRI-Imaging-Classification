...
## Conclusions
During the first few iterations of the model, the primary difficulties encountered were 1) the oscillating cross-validation loss/accuracy between epochs and 2) the relatively low cross-validation accuracy of the model. Regarding the first problem, it was believed that the oscillation originated from the extreme underrepresentation of the ModerateDemented class. This may have resulted in a fluctuating frequency of ModerateDemented cases selected for each batch, and combined with the large class weight assigned to ModerateDemented, this class may have exerted a large influence on parameter updates. To alleviate this issue, the learning rate was reduced ten times (from 0.001 to 0.0001), gradient clipping was implemented, and a stratified val/training split was used, amongst other changes. Additionally, to reduce the probability of overfitting, AdaptiveAvgPool2D was incorporated to reduce each feature map from the last block to a 1x1 spatial representation, reducing the size of the vector input into the linear layers and thus reducing the overall complexity of the model. Although these changes resulted in a slight increase in validation and training accuracy (from about ~50% to ~60%), it was discovered that by increasing the number of epochs from 20 to 50, the oscillation diminished in magnitude, suggesting that the early instability observed could potentially be due to the model not sufficiently converging, rather than the architecture. 
